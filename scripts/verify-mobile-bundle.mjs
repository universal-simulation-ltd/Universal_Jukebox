...
//
// The fault: Universal Jukebox is hosted at opensource.unisim.co.uk/jukebox/, so the
// production build sets Vite's `base` to `/qr/` and index.html asks for
// `/jukebox/assets/index-….js`. Capacitor serves the bundle from `capacitor://localhost`,
// whose document root IS this copied directory — there is no `/jukebox/` inside it, so
