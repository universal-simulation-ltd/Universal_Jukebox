...
import { describeNativeNowPlaying } from './nowPlayingNative'
...
function watchBackgroundPlayback(): void {
  ...
  document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
      ...
      noteEvent('media', { session: describeMediaSession(), fx: effectsState(), lockArt: describeNativeNowPlaying() })
      console.log(`[jukebox:bg] media session ${describeMediaSession()} fx=${effectsState()} lockArt=${describeNativeNowPlaying()}`)
